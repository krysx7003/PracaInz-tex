...
{\noindent \large \textbf{Pytanie 1}}
    ...
    Wyniki obserwowane w tabeli \ref{tab:dataset_comparison}, szczególnie te dla autorskiego zbioru
    oraz zbioru old-books, zapewniają silne statystycznie różnicowanie algorytmów (wysokie $\chi^2$ oraz
    $p<0.0001$). Trzecia wersja autorskiego zbioru zapewnia największą różnicę między najlepszym a
    najgorszym algorytmem. Oznacza to, że zbiór ten jest najbardziej czuły na różnice w algorytmach.
    Zbiór IAM ma najniższą statystykę $\chi^2$ przy jednocześnie wysokim średnim CER, co sugeruje, że
    jest trudny, ale słabiej rozróżnia algorytmy. To bezpośrednio wspiera tezę, że odpowiednio
    zaprojektowana wersja autorskiego zbioru (tu: wersja 2 lub 3) może być lepszym narzędziem do porównywania
    algorytmów niż standardowe benchmarki.
    \vspace{0.5em}
...
{\noindent \large \textbf{Pytanie 2}}
    ...
    najlepszą efektywność osiąga algorytm Paddle. Szczególnie dobrze widać to w przypadku, w którym
    skorzystano z kroju Allura (tabela \ref{tab:hand_connected_summary}), gdzie algorytm ten otrzymał
    średnie CER na poziomie 0.0775. Jednoznacznie dominując nad konkurencją, drugim w kolejności był
    ...
    jako jedyny z wybranych umożliwiał wykorzystanie akceleracji przy pomocy procesora graficznego(GPU). Ta
...
{\noindent \large \textbf{Pytanie 3}}
    ...
    dominacja nie była tak jednoznaczna. W przypadku obrazów zacienionych DocTR otrzymał wyniki istotnie
    niższe od pozostałych(tabela \ref{tab:shadow_summary}). W pozostałych przypadkach różnica między algorytmem
    DocTR a pozostałymi nie była aż tak widoczna (np. w tabeli \ref{tab:tilt_summary} różnica między DocTR a Paddle
    to jedynie 0.01).

    Obserwowana wyższość algorytmu DocTR nad pozostałymi była, w tym przypadku, wynikiem oczekiwanym. DocTR
    jest algorytmem trenowanym do rozpoznawania obrazów bliższych rzeczywistości. Dla przykładu, zbiór treningowy
    ...
    czterema powtórzeniami. W wyniku tego działania potwierdzono, że dane otrzymane w kolejnych powtórzeniach
    ...
    tłem. Jednakże potwierdzenie tej teorii wymaga dalszych badań.
    \vspace{0.5em}
    
Warte odnotowania są także średnie czasy analizy obrazu. Algorytm Paddle OCR uzyskiwał istotnie niższe wyniki 
...
Tesseract, który otrzymywał wyniki bliższe algorytmowi Paddle, podczas gdy czas pozostawał zbliżony do drugiego
algorytmu. Jednakże w przeciwieństwie do algorytmu Paddle, Tesseract nie potrzebuje przyspieszenia GPU, więc
może być bardziej użyteczny w środowiskach, w których procesor graficzny jest niedostępny.
